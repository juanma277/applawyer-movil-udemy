...
export class LoginPage implements OnInit {
  ...
  signInWithGoogle(){
    this.googlePlus.login({
      ...
      .then( user => {
        this.usuarioService.cargarUsuario(user.displayName, user.email,user.photoURL, user.uid, 'Google', 'USER_ROLE', '', '', 0,1);
    ...
    if (this.platform.is('cordova')) {
      ...
      this.fb.login(['email', 'public_profile']).then(res => {
        ...
        firebase.auth().signInWithCredential(facebookCredential).then(user=>{
          this.usuarioService.cargarUsuario(user.displayName, user.email,user.photoURL, user.uid, 'Facebook', 'USER_ROLE', '', '', 0,1);
          this.usuarioService.guardarStorage();
          //this.navCtrl.setRoot(HomePage);
        }).catch(error =>{
