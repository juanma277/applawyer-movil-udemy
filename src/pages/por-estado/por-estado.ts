...
export class PorEstadoPage implements OnInit {
...
ngOnInit() {
  ...
  this.procesoService.procesosPorEstado(this.user_id).subscribe((resp:any)=>{
    this.procesos = resp.process;
    for(let y=0; y< resp.process.length; y++){
      this.procesosCantidad.push(resp.process[y].cantidad);
      this.juzgadosNombre.push(resp.process[y].estado);
